...
class Swiper extends Component {
    ...
    autoPlay() {
        ...
        this.intervalId = setInterval(() => {
            const { displayMultipleItems } = this.props;
            let current = (this.current + 1) % (this.items.length - displayMultipleItems + 1);
            this.translateXStart = this.translateX;
            if (!this.touching) {
                let cb = () => {};
    ...
    renderIndicator() {
        ...
        const getStyle = index => ({
            backgroundColor:
                activeIndexs.indexOf(index) !== -1 ? indicatorActiveColor : indicatorColor
        });
        return (
    ...
    translate(distance, sign = true) {
        ...
        this.wrapper.style.transform = `translate${vertical ? 'Y' : 'X'}(${distance}px)`;
    ...
    componentDidUpdate() {
        ...
        this.swiperLength = vertical ? this.swiper.offsetHeight : this.swiper.offsetWidth;
        // 更新 translateRange
        if (this.items.length !== 0 && this.swiperLength) {
            this.translateRange.min =
                -this.swiperLength * ((1 / displayMultipleItems) * this.items.length - 1);
        }
    }
    ...
    render() {
        ...
        return (
            <div ref={s => (this.swiper = s)} className={`h5-swiper ${propClass}`} style={style}>
                <div
                    ref={w => (this.wrapper = w)}
                    className={`h5-swiper-wrapper ${vertical ? 'h5-swiper-wrapper-vertical' : ''}`}
